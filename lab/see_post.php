<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["delete_id"])) {
        ...
        // Prepare and execute the SQL query to delete the post from the database
        $post_stmt = $conn->prepare("DELETE FROM post WHERE id = ?");
        $post_stmt->bind_param("i", $delete_id);
        $post_stmt->execute();
        $post_stmt->close();
    
        // Prepare and execute the SQL query to delete from the recruiters table
        $recruiters_stmt = $conn->prepare("DELETE FROM recruiters WHERE post_id = ?");
        $recruiters_stmt->bind_param("i", $delete_id);
        $recruiters_stmt->execute();
        $recruiters_stmt->close();
    
        // Prepare and execute the SQL query to delete from the applicants table
        $applicants_stmt = $conn->prepare("DELETE FROM applicants WHERE post_id = ?");
        $applicants_stmt->bind_param("i", $delete_id);
        $applicants_stmt->execute();
        $applicants_stmt->close();
    
        // Redirect to the same page after successful deletion to update the table
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    if ($filter_category === '' || $row['category'] === $filter_category) {
                                        if ($row['name'] === $_SESSION['username']) {
                                            ...
                                            echo "<td class='delete'><a href='" . $_SERVER['PHP_SELF'] . "?delete_id=" . $row['id'] . "'><button><i class='fas fa-trash-alt'></i></button></a></td>";
                                            echo "</tr>";

                                            $count++;
                                        }
                                    }
                                    // Check if the username from the database matches the session's username
                                    
                                }
                                if ($count === 1) {
                                    echo "<tr><td colspan='4'>No posts found for the current user</td></tr>";
                                }
                            } else {
